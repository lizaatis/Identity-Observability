...
// IdentityResolution handles matching Okta users to canonical identities
...
type LowConfidenceMatch struct {
	OktaUserID      string
	OktaEmail       string
	OktaEmployeeID  string
	CanonicalID     int64
	CanonicalEmail  string
	CanonicalEmpID  string
	Confidence      float64
	MatchMethod     string // "employee_id", "email", "upn"
}

// ResolveIdentity matches an Okta user to a canonical identity
// Returns: (canonical_identity_id, confidence_score, match_method, error)
func (ir *IdentityResolution) ResolveIdentity(ctx context.Context, oktaUserID, email, employeeID, upn string) (int64, float64, string, error) {
	...
	if employeeID != "" {
		...
		if err == nil {
			ir.recordMatch(oktaUserID, canonicalID, 1.0, "employee_id", email, employeeID)
	...
	if email != "" {
		...
		if err == nil {
			...
			ir.recordMatch(oktaUserID, canonicalID, confidence, "email", email, employeeID)
	...
	// Method 3: Match by UPN (if different from email) (medium confidence: 0.7)
	if upn != "" && upn != email {
		var canonicalID int64
		err := ir.pool.QueryRow(ctx, `
			SELECT id FROM identities WHERE LOWER(email) = LOWER($1)
		`, upn).Scan(&canonicalID)
		
		if err == nil {
			ir.recordMatch(oktaUserID, canonicalID, 0.7, "upn", email, employeeID)
			return canonicalID, 0.7, "upn", nil
		} else if err != pgx.ErrNoRows {
			return 0, 0, "", err
		}
	}
...
func (ir *IdentityResolution) recordMatch(oktaUserID string, canonicalID int64, confidence float64, method, email, employeeID string) {
