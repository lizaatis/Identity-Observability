...
// RateLimiter handles Okta API rate limiting with exponential backoff
...
func (rl *RateLimiter) WaitIfNeeded(ctx context.Context) error {
	...
	// Okta rate limit: typically 600 requests per minute
	// Be conservative: wait if we've made 500 requests in the window
	if rl.requestCount >= 500 {
