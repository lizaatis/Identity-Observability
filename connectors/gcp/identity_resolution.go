...
// IdentityResolution handles matching GCP users to canonical identities
...
// ResolveIdentity matches a GCP user to a canonical identity
// Returns: (canonical_identity_id, confidence_score, match_method, error)
func (ir *IdentityResolution) ResolveIdentity(ctx context.Context, gcpUserID, email, employeeID string) (int64, float64, string, error) {
	...
	if employeeID != "" {
		...
		if err == nil {
			ir.recordMatch(gcpUserID, canonicalID, 1.0, "employee_id", email, employeeID)
	...
	if email != "" {
		...
		if err == nil {
			...
			ir.recordMatch(gcpUserID, canonicalID, confidence, "email", email, employeeID)
...
func (ir *IdentityResolution) recordMatch(gcpUserID string, canonicalID int64, confidence float64, method, email, employeeID string) {
